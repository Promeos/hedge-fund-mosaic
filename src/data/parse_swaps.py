...
def parse_all_swaps(data_dir: str | None = None, output_dir: str | None = None) -> None:
    ...
    wide = combined.pivot_table(index="date", columns="metric", values="value_billions", aggfunc="first")
    ...
    q_agg["ir_total_mean"] = ("ir_total", "mean") if "ir_total" in wide.columns else None

    quarterly = (
        wide.groupby("quarter")
        .agg(weeks=("date", "count"), **{col: (col, "last") for col in wide.columns if col not in ["date", "quarter"]})
        .reset_index()
    )
    ...
    if "credit_total" in wide.columns:
        print(
            f"  Credit notional: ${latest['credit_total']:,.0f}B (cleared: {latest.get('credit_cleared_pct', 0):.1%})"
        )
