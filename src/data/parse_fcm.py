...
def parse_single_fcm_file(filepath: str) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        data,
        columns=col_names[:max_cols]
        if max_cols <= len(col_names)
        else col_names + [f"col_{i}" for i in range(len(col_names), max_cols)],
    )
...
def parse_all_fcm(data_dir: str | None = None, output_dir: str | None = None) -> None:
    ...
    money_cols = [
        ...
        "excess_net_capital",
        "customer_assets_seg",
        "customer_seg_required",
        "excess_funds_seg",
        "funds_section_30_7",
        "customer_pt30_required",
        "cleared_swap_seg",
        "cleared_swap_required",
        "excess_cleared_swap",
    ...
    if "customer_assets_seg" in fcm_all.columns:
        ...
        for date, group in fcm_all.groupby("as_of_date"):
            ...
            top_brokers.append(
                top[["as_of_date", "fcm_name", "adj_net_capital", "customer_assets_seg", "market_share_seg"]]
            )
