...
def load_all_sources():
    ...
    if 'swaps_weekly' in sources:
        sources['swaps_weekly']['date'] = pd.to_datetime(
            sources['swaps_weekly']['date'], errors='coerce'
        )
    if 'fcm' in sources:
        sources['fcm']['as_of_date'] = pd.to_datetime(sources['fcm']['as_of_date'])
    if 'fcm_monthly' in sources:
        sources['fcm_monthly']['as_of_date'] = pd.to_datetime(
            sources['fcm_monthly']['as_of_date']
        )
    if 'fcm_concentration' in sources:
        sources['fcm_concentration']['as_of_date'] = pd.to_datetime(
            sources['fcm_concentration']['as_of_date']
        )
    if 'dtcc' in sources:
        sources['dtcc']['date'] = pd.to_datetime(
            sources['dtcc']['date'], errors='coerce'
        )
...
def align_quarterly(sources):
    ...
    if 'z1' in sources:
        ...
        keep_cols = [
            ...
            'Derivatives (long value)', 'leverage_ratio', 'leverage_change',
    ...
    if 'swaps_quarterly' in sources:
        ...
        swap_cols = [
            ...
            'ir_cleared_pct', 'credit_cleared_pct', 'fx_cleared_pct',
        ]
    ...
    if 'fcm' in sources:
        ...
        fcm.index.name = 'date'
        fcm_cols = [
            'adj_net_capital', 'excess_net_capital', 'customer_assets_seg',
            'cleared_swap_seg', 'fcm_count', 'capital_adequacy_ratio',
            'excess_capital_pct', 'swap_seg_share',
        ]
        available = [c for c in fcm_cols if c in fcm.columns]
        fcm_aligned = fcm[available].copy()
        # Convert from raw USD to billions
        usd_cols = [
            'adj_net_capital', 'excess_net_capital',
            'customer_assets_seg', 'cleared_swap_seg',
        ]
        for c in usd_cols:
    ...
    print(f"Aligned quarterly dataset: {aligned.shape[0]} quarters, "
...
def reconcile_z1_formpf(aligned):
    ...
    print(f"Z.1 vs Form PF reconciliation: ratio mean={result['asset_ratio']['mean']:.3f}, "
...
def compute_cross_metrics(aligned):
    ...
    if 'pf_gav_nav_ratio' in df.columns and 'z1_leverage_ratio' in df.columns:
        df['implied_leverage_gap'] = (
            df['pf_gav_nav_ratio'] - df['z1_leverage_ratio']
        )
...
def _make_result(test_id, description, statistic=np.nan, p_value=np.nan,
...
def test_adf_stationarity(series, name='series'):
    ...
    try:
        ...
        if len(clean) < 8:
            return _make_result(
                'ADF', f'ADF stationarity: {name}',
                interpretation='Insufficient data (< 8 obs)',
            )
        result = adfuller(clean, autolag='AIC')
        adf_stat, p_val = result[0], result[1]
        interp = (f'Stationary at 5% (ADF={adf_stat:.3f})'
                  if p_val < 0.05
                  else f'Non-stationary (ADF={adf_stat:.3f})')
        return _make_result('ADF', f'ADF stationarity: {name}',
                            statistic=adf_stat, p_value=p_val,
...
def test_mann_kendall(series, name='series'):
    ...
    try:
        ...
        return _make_result('MK', f'Mann-Kendall trend: {name}',
                            statistic=tau, p_value=p_val,
                            interpretation=interp)
    except Exception as exc:
        return _make_result('MK', f'Mann-Kendall trend: {name}',
                            interpretation=f'Error: {exc}')
    ...
    try:
        ...
        if not all(c in aligned.columns for c in cols):
            return _make_result(test_id, desc,
                                interpretation='Missing columns')
        overlap = aligned[cols].dropna()
        if len(overlap) < 10:
            return _make_result(test_id, desc,
                                interpretation='Insufficient data (< 10 obs)')
        coint_stat, p_val, crit_values = coint(
            overlap['z1_Total assets'], overlap['pf_gav'],
        )
        interp = ('Cointegrated at 5%' if p_val < 0.05
                   else 'Not cointegrated at 5%')
        return _make_result(test_id, desc, statistic=coint_stat,
                            p_value=p_val, interpretation=interp)
    except Exception as exc:
...
def test_h2_ratio_stability(aligned):
    ...
    try:
        ...
        interp = ('Ratio is stationary (stable relationship)'
                  if p_val < 0.05
                  else 'Ratio is non-stationary (relationship drifts)')
        r = _make_result(test_id, desc, statistic=adf_stat,
                         p_value=p_val, interpretation=interp)
...
def test_h3_cleared_pct_equivalence(aligned):
    ...
    try:
        ...
        if dtcc_col is None or swap_col not in aligned.columns:
            return _make_result(test_id, desc,
                                interpretation='Missing DTCC or swap cleared pct data')
        ...
        interp = ('Equivalent within 10pp' if p_tost < 0.05
                   else 'Cannot confirm equivalence within 10pp')
        r = _make_result(test_id, desc, statistic=mean_diff,
                         p_value=p_tost, interpretation=interp)
...
def test_h4_leverage_granger(aligned):
    ...
    try:
        ...
        if len(overlap) < 12:
            return _make_result(test_id, desc,
                                interpretation='Insufficient data (< 12 obs)')
        ...
        interp = (f'Granger-causal at lag {best_lag} (F={f_stat:.2f})'
                  if min_p < 0.05
                  else f'No Granger causality (best lag={best_lag})')
        return _make_result(test_id, desc, statistic=f_stat,
                            p_value=min_p, interpretation=interp)
    except Exception as exc:
        return _make_result(test_id, desc, interpretation=f'Error: {exc}')
...
def test_h5_fcm_leads_cot(aligned):
    ...
    try:
        ...
        if fcm_col not in aligned.columns or cot_col not in aligned.columns:
            return _make_result(test_id, desc,
                                interpretation='Missing FCM or COT columns')
        ...
        cot_growth = overlap[cot_col].pct_change().replace(
        ...
        if len(common_idx) < 6:
            return _make_result(test_id, desc,
                                interpretation='Insufficient overlapping growth data')
        ...
        direction = 'FCM leads COT' if best_lag > 0 else (
            'COT leads FCM' if best_lag < 0 else 'Contemporaneous')
        interp = (f'{direction} by {abs(best_lag)}Q (r={max_corr:.3f})'
                  if not np.isnan(p_val) and p_val < 0.05
                  else f'No significant lead-lag (best lag={best_lag}, r={max_corr:.3f})')
        return _make_result(test_id, desc, statistic=max_corr,
                            p_value=p_val, interpretation=interp)
    except Exception as exc:
        return _make_result(test_id, desc, interpretation=f'Error: {exc}')
...
def test_h6_liquidity_vix(sources):
    ...
    try:
        if 'form_pf_liquidity' not in sources or 'vix' not in sources:
            return _make_result(test_id, desc,
                                interpretation='Missing liquidity or VIX data')
        ...
        inv = liq[(liq['liquidity_type'] == 'investor_liquidity')
                  & (liq['period'] == 'At most 30 days')].copy()
        port = liq[(liq['liquidity_type'] == 'portfolio_liquidity')
                   & (liq['period'] == 'At most 30 days')].copy()

        if inv.empty or port.empty:
            return _make_result(test_id, desc,
                                interpretation='Could not compute liquidity mismatch')
        ...
        if 'VIX_max' not in vix.columns:
            ...
            if not vix_col:
                return _make_result(test_id, desc,
                                    interpretation='Cannot identify VIX max column')
        ...
        interp = (f'30-day liquidity gap is higher in high-VIX quarters '
                  ...
                  if p_val < 0.05
                       ...
                       f'(high={high_mismatch.mean():.3f}, low={low_mismatch.mean():.3f})')
        return _make_result(test_id, desc, statistic=t_stat,
                            p_value=p_val, interpretation=interp)
    except Exception as exc:
        return _make_result(test_id, desc, interpretation=f'Error: {exc}')
...
def test_h7_concentration_correlation(sources):
    ...
    try:
        if 'form_pf_concentration' not in sources:
            return _make_result(test_id, desc,
                                interpretation='Missing Form PF concentration data')
        ...
        if top10.empty:
            return _make_result(test_id, desc,
                                interpretation='No Top 10 data in Form PF concentration')
        ...
        if not os.path.exists(thirteenf_path):
            ...
            tau, p_val = stats.kendalltau(
                np.arange(len(top10)), top10['nav_share'].values,
            )
            interp = (f'Form PF concentration trend: tau={tau:.3f} '
                      f'(no 13F concentration available for cross-source test)')
            return _make_result(test_id, desc, statistic=tau,
                                p_value=p_val, interpretation=interp)
        ...
        if not hhi_13f:
            return _make_result(test_id, desc,
                                interpretation='Could not compute 13F HHI')
        ...
        if len(merged) < 4:
            return _make_result(test_id, desc,
                                interpretation='Insufficient overlapping quarters')

        rho, p_val = stats.spearmanr(merged['nav_share'], merged['hhi_13f'])
        interp = (f'Significant correlation (rho={rho:.3f})'
                  if p_val < 0.05
                  else f'No significant correlation (rho={rho:.3f})')
        return _make_result(test_id, desc, statistic=rho,
                            p_value=p_val, interpretation=interp)
    except Exception as exc:
        return _make_result(test_id, desc, interpretation=f'Error: {exc}')
    ...
    try:
        ...
        if not all(c in aligned.columns for c in cols):
            ...
            if not all(c in aligned.columns for c in alt):
                return _make_result(test_id, desc,
                                    interpretation='Missing VIX or leverage columns')
            # Use leverage diff
        ...
        f_stat = gc_results[best_lag][0]['ssr_ftest'][0]
        interp = (f'VIX Granger-causes leverage change at lag {best_lag} (F={f_stat:.2f})'
                  if min_p < 0.05
                  else f'No Granger causality (best lag={best_lag})')
        return _make_result(test_id, desc, statistic=f_stat,
                            p_value=min_p, interpretation=interp)
...
def run_all_tests(aligned, sources):
    ...
    summary_cols = ['test_id', 'description', 'statistic', 'p_value',
    ...
    for _, row in df.iterrows():
        ...
        print(f"  [{row['result']:>4s}] {row['test_id']:>4s}: "
              f"{row['description'][:55]:<55s} {p_str}")
    print(f"{'='*80}\n")
    return df
